...
admin.site.index_title = "لوحة التحكم بالمحتوى التعليمي"

# --- كود JavaScript المطور لإظهار زر الرسم وتفعيل الإدراج ---
MATHLIVE_JS = """
<script src="https://unpkg.com/mathlive"></script>
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // إعدادات لوحة المفاتيح لدعم الرسم (Handwriting)
        ...
        window.openMathDrawer = function(targetId) {
            ...
            overlay.innerHTML = `
                <div style="background:white; padding:25px; border-radius:15px; width:95%; max-width:800px; box-shadow: 0 20px 60px rgba(0,0,0,0.5); direction:rtl; text-align:right;">
                    <h3 style="color:#2d5a27; margin-bottom:15px; font-family:sans-serif;">🎨 لوحة الرسم والرموز الرياضية</h3>
                    ...
                    <p style="text-align:center; color:#666; margin-top:10px; font-size:14px;">💡 انقري داخل المربع ثم استخدمي أيقونة "القلم" في اللوحة السفلية للرسم بالفأرة.</p>
                </div>
            `;
            ...
            // تفعيل الحقل وإظهار الكيبورد فوراً
            setTimeout(() => {
                mf.focus();
                mathVirtualKeyboard.show();
            }, 300);

            // وظيفة الإدراج في CKEditor
        ...
        // حقن أزرار الفتح تحت محررات النصوص
        function injectButtons() {
            document.querySelectorAll('.django-ckeditor-widget').forEach(widget => {
                ...
                if (!widget.querySelector('.math-draw-btn')) {
                    ...
                    btn.innerHTML = '➕ إضافة رموز أو رسم رياضي';
                    btn.type = 'button';
                    btn.style = "margin-top:10px; background:#2d5a27; color:white; padding:8px 20px; border-radius:25px; cursor:pointer; font-weight:bold; border:none; font-size:13px;";
...
# --- إعداد الأسئلة لتظهر داخل المهارة ---
class QuestionInline(admin.StackedInline):
    model = Question
    extra = 1
    classes = ['collapse']
    verbose_name = "سؤال تدريبي"
    verbose_name_plural = "🧩 بنك الأسئلة السريع"

# --- تسجيل المهارات (Skill) مع التنسيق الأنيق ---
@admin.register(Skill)
class SkillAdmin(admin.ModelAdmin):
    list_display = ('title', 'category')
    list_filter = ('category', 'teachers')
    fieldsets = (
        ('📌 معلومات المهارة', {'fields': ('title', 'category', 'teachers', 'required_questions_count')}),
        ('📖 محتوى الشرح', {'fields': ('content_text', 'video_url', 'pdf_file', 'image_explainer')}),
        ('🎨 إعدادات المظهر', {'fields': ('icon_image', 'card_color', 'short_description'), 'classes': ('collapse',)}),
    )
...
# --- تسجيل الأسئلة (Question) بشكل مستقل ليظهر الزر في القائمة الجانبية ---
